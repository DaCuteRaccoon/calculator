...
pub fn equation() {
  ...
  let mut infix = get_input::read("Enter expression: ").replace("pi","(3.141592653589793238462643383279)").replace("**","^").replace("x","*").replace(")(",")*(").
  ...
  if infix.contains(&">".to_string()) || infix.contains(&"=".to_string()) || infix.contains(&"<".to_string()) {
    ...
    if sol >= 1000000000000.0 {
      solution = format!("{:e}",sol);
    }
    let last = solution[0..solution.len()].chars().last();
